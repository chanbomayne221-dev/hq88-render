...
// __dirname tương thích cả CommonJS lẫn ESM bundling
...
function isFakeBotEnabled() {
  const row = db.prepare(
    "SELECT value FROM bot_settings WHERE key='fake_bot_enabled'"
  ).get() as any;
  return !row || row.value === "1";
}

function setFakeBotEnabled(enabled: boolean) {
  db.prepare(`
    INSERT INTO bot_settings (key, value)
    VALUES ('fake_bot_enabled', ?)
    ON CONFLICT(key)
...
// ─── VIP helpers (1 điểm VIP = 250.000đ tiền cược) ─────────────────────────
const VIP_POINT_VALUE = 250_000;
const VIP_THRESHOLDS: Array<{ level: number; points: number }> = [
  { level: 9, points: 100_000 },
  { level: 8, points: 50_000 },
  { level: 7, points: 10_000 },
  { level: 6, points: 5_000 },
  { level: 5, points: 1_000 },
  { level: 4, points: 500 },
  { level: 3, points: 100 },
  { level: 2, points: 50 },
  { level: 1, points: 10 },
];
function vipLevelFromBet(totalBet: number): number {
  const points = Math.floor((totalBet ?? 0) / VIP_POINT_VALUE);
  for (const t of VIP_THRESHOLDS) if (points >= t.points) return t.level;
  return 0;
}
function syncVipLevel(u: any) {
  if (!u) return u;
  const lvl = vipLevelFromBet(u.total_bet ?? 0);
  if (lvl !== u.vip_level) {
    try { db.prepare("UPDATE users SET vip_level = ? WHERE id = ?").run(lvl, u.id); } catch {}
    u.vip_level = lvl;
  }
  return u;
}
...
function getUserByTelegramId(telegramId: number) {
  const u = db.prepare("SELECT * FROM users WHERE telegram_id = ?").get(telegramId) ?? null;
  return syncVipLevel(u as any);
}

function getUserById(id: number) {
  const u = db.prepare("SELECT * FROM users WHERE id = ?").get(id) ?? null;
  return syncVipLevel(u as any);
}
...
// Trả về giờ VN (GMT+7) định dạng dd/MM/yyyy HH:mm:ss
function vnTime(d: Date = new Date()) {
...
// Migration: bổ sung cột max_*_streak cho DB đã tồn tại (bỏ qua nếu đã có)
try { db.exec(`ALTER TABLE users ADD COLUMN max_win_streak INTEGER NOT NULL DEFAULT 0`); } catch {}
try { db.exec(`ALTER TABLE users ADD COLUMN max_lose_streak INTEGER NOT NULL DEFAULT 0`); } catch {}
// Đồng bộ top với dây hiện tại (lần đầu sau khi thêm cột)
try { db.exec(`UPDATE users SET max_win_streak = win_streak WHERE max_win_streak < win_streak`); } catch {}
try { db.exec(`UPDATE users SET max_lose_streak = lose_streak WHERE max_lose_streak < lose_streak`); } catch {}
...
function updateVipLevel(user: any) {
  // Cấp VIP được tính theo TIỀN CƯỢC: 1 điểm VIP = 250.000đ cược.
  const newLevel = vipLevelFromBet(user.total_bet ?? 0);
  if (newLevel !== user.vip_level) {
    try { db.prepare("UPDATE users SET vip_level = ? WHERE id = ?").run(newLevel, user.id); } catch {}
...
async function endSession(chatId: number, sessionId: number, forceDice?: [number|undefined, number|undefined, number|undefined]) {
  ...
  for (const { telegramId, betType, amount, payout, newBal, displayName } of winnerMessages) {
    ...
    if (jp) {
      ...
      try { await bot.sendMessage(telegramId,
        ...
        `✔️ Thắng cược (${betTypeName[betType] ?? betType}): *+${formatNumber(payout)}*\n` +
  ...
  for (const { telegramId, betType, amount, currentBal } of loserMessages) {
    if (telegramId < 0) continue; // skip fake bots
    try { await bot.sendMessage(telegramId, `⛔ Thua −${formatNumber(amount)} ${betTypeName[betType] ?? betType}\n phiên #${session.sessionNumber}\n Số dư: ${formatNumber(currentBal)}`); } catch {}
  }
...
async function handleSlGame(msg: TelegramBot.Message): Promise<boolean> {
  ...
  if (isJackpot) {
    ...
    db.prepare("UPDATE users SET balance=? WHERE id=?").run(nb, user.id);
  ...
  try {
    await bot.sendMessage(chatId,
      ...
      { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "♻️ Quay lại", callback_data: "sl_play_again" }]] } }
    );
...
async function handleBrGame(msg: TelegramBot.Message): Promise<boolean> {
  ...
  try {
    await bot.sendMessage(chatId,
      `🏀 *KẾT QUẢ BÓNG RỔ*\n\n${resultText}`,
      { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "♻️ Chơi lại", callback_data: "br_play_again" }]] } }
    );
...
async function handleSbGame(msg: TelegramBot.Message): Promise<boolean> {
  ...
  if (won) {
    ...
    resultText = `🥂 *TRÚNG SỐ!* Tổng: *${total}* = *${chosenNum}* ✔️\n💎 Thắng *+${formatNumber(payout)}đ* (x${multiplier})\n💎 Số dư: *${formatNumber(nb)}đ*`;
  } else {
  ...
  try {
    await bot.sendMessage(
      ...
      { parse_mode: "Markdown", reply_to_message_id: msg.message_id, allow_sending_without_reply: false, reply_markup: { inline_keyboard: [[{ text: "♻️ Chơi lại", callback_data: `sb_again_${chosenNum}_${amount}` }]] } }
    );
...
async function resolveXxGame(chatId: number, userId: number, betType: string, amount: number, total: number, dice: number[], balAfterBet: number) {
  ...
  if (won) {
    ...
    resultText = `🥂 *THẮNG!* Tổng: *${total}* → *${XX_LABEL[betType]}* ✔️\n💎 Thắng *+${formatNumber(payout)}đ*\n💎 Số dư: *${formatNumber(nb)}đ*`;
  } else {
  ...
  try {
    await bot.sendMessage(chatId,
      ...
      { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "♻️ Chơi lại", callback_data: "xx_play_again" }]] } }
    );
...
async function sendAccountInfo(chatId: number, telegramId: number) {
  const user = getUserByTelegramId(telegramId);
  if (!user) return;
  // Ngày tham gia (định dạng VN dd/mm/yyyy)
  let joinDate = "—";
  try {
    const d = new Date((user.created_at as string).replace(" ", "T") + "Z");
  ...
  await bot.sendMessage(chatId, msg, { reply_markup: accountInlineKeyboard(), parse_mode: "HTML" } as any);
}
...
async function sendBetHistory(chatId: number, userId: number) {
  ...
  const lines = rows.map((r, i) => `${i + 1}. ${formatVNDateTime(r.ended_at)} | ${shortLabel[r.bet_type] ?? r.bet_type} | ${formatNumber(r.amount)} | ${r.is_win ? "Thắng ✔️" : "Thua ⛔"}`);
...
function hasFirstDeposit(user: any) {
  return user.first_deposit_done === 1;
}
...
async function redeemGiftcode(chatId: number, telegramId: number, code: string) {
  const user = getUserByTelegramId(telegramId);
  if (!user) return;
  // Cho phép nhập giftcode kể cả khi chưa nạp lần đầu (theo yêu cầu)
...
async function handleCheckin(chatId: number, telegramId: number) {
  ...
  if (isSameDay(user.last_checkin, today)) { await bot.sendMessage(chatId, "✔️ Bạn đã điểm danh hôm nay rồi. Quay lại vào ngày mai!", mainMenuKeyboard()); return; }
...
async function sendAdminUserList(chatId: number, page = 1) {
  ...
  const lines = users.map((u) => `${u.is_blocked ? "🔒" : "✔️"} ID:${u.id} | ${u.first_name ?? "Ẩn danh"}${u.username ? ` (@${u.username})` : ""} | 💎${formatNumber(u.balance)} | VIP${u.vip_level}`);
...
async function sendAdminUserInfo(chatId: number, targetId: number, editMessageId?: number) {
  ...
  const keyboard = {
    inline_keyboard: [
      ...
      [{ text: "♻️ Làm mới", callback_data: `adm_user_refresh_${user.id}` }, { text: "⛔ Tắt", callback_data: "adm_close" }],
...
async function sendAdminGiftList(chatId: number, editMessageId?: number) {
  ...
  const lines = gifts.map((g) => { const status = !g.is_active ? "⛔Tắt" : g.used_count >= g.max_uses ? "🔴Hết" : "✔️Hoạt động"; const exp = g.expires_at ? ` | hết:${g.expires_at.slice(0, 10)}` : ""; return `${status} *${g.code}* | ${formatNumber(g.amount)} | ${g.used_count}/${g.max_uses}${exp}`; });
  ...
  rows.push([{ text: "♻️ Làm mới", callback_data: "adm_listgifts_refresh" }, { text: "🔙 Menu Admin", callback_data: "adm_menu" }]);
...
// ─── startBot ─────────────────────────────────────────────────────────────────
export async function startBot(): Promise<TelegramBot | null> {
  const token = process.env["TELEGRAM_BOT_TOKEN"];
  if (!token) { logger.warn("TELEGRAM_BOT_TOKEN không được cấu hình, bỏ qua khởi động bot"); return null; }

  // Khởi tạo KHÔNG auto-poll để chủ động xoá webhook + drop pending updates
  // trước khi bắt đầu polling. Tránh lỗi 409 Conflict khi Render redeploy
  // hoặc có instance cũ còn đang giữ getUpdates.
  bot = new TelegramBot(token, { polling: false });
  setBotInstance(bot);

  // Xoá webhook + drop các update đang chờ (nếu có)
  try {
    await bot.deleteWebHook({ drop_pending_updates: true } as any);
    logger.info("Đã xoá webhook cũ và drop pending updates");
  } catch (e: any) {
    logger.warn({ err: String(e?.message || e) }, "deleteWebHook lỗi (bỏ qua)");
  }

  // Chờ 2s để Telegram giải phóng session getUpdates cũ (instance cũ)
  await new Promise((r) => setTimeout(r, 2000));

  // Bắt đầu polling với retry khi gặp 409
  const startPollingWithRetry = async (attempt = 1): Promise<void> => {
    try {
      await bot.startPolling({ restart: true });
    } catch (err: any) {
      const msg = String(err?.message || err);
      if (/409|Conflict|terminated by other getUpdates/i.test(msg) && attempt <= 5) {
        const wait = Math.min(30000, 5000 * attempt);
        logger.warn(`409 Conflict – chờ ${wait}ms rồi thử lại (lần ${attempt}/5)`);
        await new Promise((r) => setTimeout(r, wait));
        return startPollingWithRetry(attempt + 1);
      }
      throw err;
    }
  };
  ...
  bot.onText(/^\/doiphientieptheo(?:@\S+)?(?:\s+(\d)(?:\s+(\d)\s+(\d))?)?$/, async (msg, match) => {
    ...
    if (match![3]) {
      ...
      try { await bot.sendMessage(chatId, "✔️ Đã hủy preset phiên tiếp theo."); } catch {}
      return;
    }

    pendingForceDice.set(groupChatId, preset);
    const desc = preset.map((v, i) => v !== undefined ? `Con ${i+1}: ${v}` : `Con ${i+1}: ngẫu nhiên`).join(" | ");
    try { await bot.sendMessage(chatId, `✔️ Đã đặt trước cho phiên tiếp theo:\n${desc}`); } catch {}
  });
  ...
  bot.onText(/^\/hoancuoc(?:@\S+)?(?:\s+(\d{4}-\d{2}-\d{2}))?$/, async (msg, match) => {
    ...
    try { await bot.sendMessage(chatId, `✔️ Đã gửi xong hoàn cược ngày *${dateLabel}*.`, { parse_mode: "Markdown" }); } catch {}
  });
  ...
  bot.onText(/\/unblock (\d+)/, async (msg, match) => {
    try {
      ...
      try { await bot.sendMessage(user.telegram_id, "✔️ Tài khoản của bạn đã được mở khóa. Chúc bạn chơi vui!"); } catch {}
    } catch (e) { console.error(e); }
  });
  ...
  bot.onText(/\/addbalance (\d+) (\d+)/, async (msg, match) => {
    try {
      ...
      await bot.sendMessage(msg.chat.id, `✔️ Đã nạp ${formatNumber(amount)} cho user ID ${targetId}. Số dư mới: ${formatNumber(newBal)}`);
  ...
  bot.onText(/\/subbalance (\d+) (\d+)/, async (msg, match) => {
    try {
      ...
      await bot.sendMessage(msg.chat.id, `✔️ Đã trừ ${formatNumber(amount)} khỏi user ID ${targetId}. Số dư mới: ${formatNumber(newBal)}`);
  ...
  bot.onText(/\/setfee (\d+) ([\d.]+)/, async (msg, match) => {
    try {
      ...
      await bot.sendMessage(msg.chat.id, `✔️ Đã đặt phí rút của user ID ${targetId} thành *${pct}%*`, { parse_mode: "Markdown" });
    } catch (e) { console.error(e); }
  });
  ...
  bot.onText(/\/delgift (\S+)/, async (msg, match) => {
    try {
      ...
      await bot.sendMessage(msg.chat.id, `✔️ Đã vô hiệu hóa giftcode *${code}*.`, { parse_mode: "Markdown" });
    } catch (e) { console.error(e); }
  });
  ...
  bot.onText(/\/broadcast (.+)/s, async (msg, match) => {
    try {
      ...
      await bot.sendMessage(msg.chat.id, `✔️ Đã gửi: ${sent} | ⛔ Thất bại: ${failed}`);
    } catch (e) { console.error(e); }
  });
  ...
  bot.onText(/^\/xoalichsu(?:@\S+)?$/, async (msg) => {
    try {
      ...
      await bot.sendMessage(msg.chat.id, "✔️ Đã xóa group lịch sử phiên.");
    } catch (e) { console.error(e); }
  });
  ...
  const sendStreakTop = async (msg: TelegramBot.Message, type: "thang" | "thua") => {
    ...
    // Top sắp xếp theo "đỉnh" (max_*_streak): dây hiện tại có thể về 0 do bị break,
    // nhưng vẫn giữ vị trí trên top cho tới khi user khác vượt qua.
    const col = type === "thang" ? "max_win_streak" : "max_lose_streak";
    const label = type === "thang" ? "THẮNG" : "THUA";
    const icon = type === "thang" ? "✔️" : "⛔";
    const limit = 3;
    const rows = db.prepare(
  ...
  bot.onText(/\/addgiftcode (\S+) (\d+)(?: (\d+))?/, async (msg, match) => {
    try {
      ...
      try {
        ...
        await bot.sendMessage(msg.chat.id, `✔️ Đã tạo giftcode *${code}* – ${formatNumber(amount)} – tối đa ${maxUses} lần.`, { parse_mode: "Markdown" });
  ...
  bot.onText(/\/batdau/, async (msg) => {
    try {
      ...
      if (enabledGroups.has(chatId)) { await bot.sendMessage(chatId, "✔️ Game tài xỉu đang chạy trong nhóm này rồi!"); return; }
  ...
  bot.onText(/\/setbank|\/caidatbank/, async (msg) => {
    try {
      ...
      await bot.sendMessage(msg.chat.id, "🏛️ Nhập tên ngân hàng (VD: MB, VCB, TCB, ACB...):");
    } catch (e) { console.error(e); }
  });
  ...
  bot.onText(/\/muagift(?:\s+(\d+))?(?:\s+(\d+))?/, async (msg, match) => {
    try {
      ...
      await bot.sendMessage(chatId, `✔️ Mua thành công ${qty} giftcode x${formatNumber(value)}\n🎁 Danh sách code (bấm vào mã để sao chép):\n${codes.map((c, i) => `${i + 1}. <code>${c}</code>`).join("\n")}`, { parse_mode: "HTML" });
    } catch (e) { console.error(e); }
  });
  ...
  bot.on("callback_query", async (query) => {
    ...
    try {
      ...
      if (data.startsWith("adm_")) {
        ...
        if (data === "adm_toggle_fakebot") {
          const current = isFakeBotEnabled();
          setFakeBotEnabled(!current);
          await bot.answerCallbackQuery(query.id, {
            text: !current ? "✔️ Đã bật bot ảo" : "⛔ Đã tắt bot ảo",
            show_alert: true,
          });
          return;
        }
        ...
        if (giftOffMatch) {
          ...
          if (g) {
            ...
            try { await bot.answerCallbackQuery(query.id, { text: `✔️ Đã tắt giftcode ${g.code}` }); } catch {}
          }
        ...
        if (data === "adm_bets_latest" || data.startsWith("adm_bets_")) {
          ...
          const lines = bets.map((b: any, i: number) => {
            ...
            const result = b.is_win ? `✔️ +${formatNumber(b.payout)}` : `⛔ -${formatNumber(b.amount)}`;
        ...
        if (unblockMatch) {
          ...
          if (u) { db.prepare("UPDATE users SET is_blocked = 0 WHERE id = ?").run(uid); await bot.sendMessage(chatId, `🔓 Đã mở khóa user ID ${uid}.`); try { await bot.sendMessage(u.telegram_id, "✔️ Tài khoản của bạn đã được mở khóa!"); } catch {} }
          return;
        }
        ...
        if (admResetConfirm) {
          ...
          db.prepare(`UPDATE users SET balance=0, total_bet=0, today_bet=0, week_bet=0, win_streak=0, lose_streak=0, max_win_streak=0, max_lose_streak=0, wager_required=0 WHERE id=?`).run(uid);
          resetState(telegramId);
          await bot.sendMessage(chatId, `✔️ Đã reset tài khoản của ${u.first_name ?? u.username ?? "user"} (ID: ${u.telegram_id}).\n\n– Số dư: 0\n– Tổng cược: 0\n– Chuỗi thắng/thua: 0\n– Vòng cược: 0`, { reply_markup: { inline_keyboard: [[{ text: "🔙 Menu Admin", callback_data: "adm_menu" }]] } });
          return;
        }
      ...
      if (data.startsWith("td_tren_") || data.startsWith("td_duoi_")) {
        ...
        if (secondTotal === firstTotal) {
          ...
          if (won) {
            ...
            db.prepare("UPDATE users SET balance=? WHERE id=?").run(newBal2, user.id);
        ...
        const playAgainKb = { reply_markup: { inline_keyboard: [[{ text: "♻️ Chơi lại", callback_data: "td_play_again" }]] } };
        try {
          await bot.sendMessage(chatId,
            `🔻 *XÚC XẮC TRÊN DƯỚI* 🔺\n\n` +
      ...
      if (data.startsWith("wit_approve_") || data.startsWith("wit_reject_")) {
        ...
        if (data.startsWith("wit_approve_")) {
          ...
          try { await bot.editMessageText(`✔️ ĐÃ DUYỆT rút *${formatNumber(wit.amount)}* cho Telegram ID ${wit.telegram_id}`, { chat_id: chatId, message_id: query.message!.message_id, parse_mode: "Markdown" }); } catch {}
          try { await bot.sendMessage(wit.telegram_id, `✔️ Rút tiền thành công!\nSố tiền ${formatNumber(wit.net)} đã được chuyển về STK liên kết của bạn`); } catch {}
          const maskedWitId = `****${String(wit.telegram_id).slice(-5)}`;
          for (const gid of enabledGroups) { try { await bot.sendMessage(gid, `*🥂 Người chơi ${maskedWitId}*\n*✔️ Rút tiền thành công ${formatNumber(wit.net)}*`, { parse_mode: "Markdown" }); } catch {} }
        } else {
          const u = db.prepare("SELECT * FROM users WHERE id = ?").get(wit.user_id) as any;
    ...
    try {
      ...
      if (text === "💳 Rút Tiền") {
        ...
        const bankInfo = (user.bank_name && user.bank_account)
          ? `🏛️ ${user.bank_name} – ${user.bank_account}${user.bank_owner ? ` – ${user.bank_owner}` : ""}`
          : `🏛️.......`;
        const msgText =
      ...
      if (state.step.startsWith("adm_")) {
        ...
        switch (state.step) {
          ...
          case "adm_subbal_amount": { const amount = parseInt(text.replace(/[.,\s]/g, "")); if (isNaN(amount) || amount <= 0) { await bot.sendMessage(chatId, "❕ Số tiền không hợp lệ."); return; } const u = getUserById(state.targetId); if (u.balance < amount) { await bot.sendMessage(chatId, `⛔ Số dư user chỉ có ${formatNumber(u.balance)}.`); resetState(telegramId); return; } const newBal = u.balance - amount; db.prepare("UPDATE users SET balance = ? WHERE id = ?").run(newBal, u.id); recordTransaction({ userId: u.id, type: "withdraw", amount, fee: 0, balanceBefore: u.balance, balanceAfter: newBal, note: "Trừ tiền (admin)" }); resetState(telegramId); await bot.sendMessage(chatId, `✔️ Đã trừ ${formatNumber(amount)} khỏi ${u.first_name ?? u.username ?? "user"} (ID: ${u.id}). Số dư mới: ${formatNumber(newBal)}`); return; }
          ...
          case "adm_set_lichsu": {
            ...
            resetState(telegramId);
            await bot.sendMessage(chatId, `✔️ Đã cài group lịch sử phiên: \`${cid}\`\n\nBot sẽ tự đăng kết quả từng phiên vào đó.`, { parse_mode: "Markdown" });
            return;
          }
          case "adm_reset_user_id": {
            ...
            await bot.sendMessage(chatId,
              ...
              { reply_markup: { inline_keyboard: [[{ text: "✔️ Xác nhận Reset", callback_data: `adm_reset_confirm_${u.id}` }, { text: "⛔ Hủy", callback_data: `adm_reset_cancel_${u.id}` }]] } }
            );
            resetState(telegramId);
            return;
          }
          ...
          case "adm_addgift_maxuses": { const maxUses = parseInt(text) || 1; try { db.prepare("INSERT INTO giftcodes (code, amount, max_uses) VALUES (?, ?, ?)").run(state.code, state.amount, maxUses); await bot.sendMessage(chatId, `✔️ Đã tạo giftcode *${state.code}* – ${formatNumber(state.amount)} – tối đa ${maxUses} lần.`, { parse_mode: "Markdown", reply_markup: adminMenuKeyboard() }); } catch { await bot.sendMessage(chatId, `⛔ Giftcode *${state.code}* đã tồn tại.`, { parse_mode: "Markdown" }); } resetState(telegramId); return; }
          case "adm_delgift_code": { const code = text.toUpperCase(); const gift = db.prepare("SELECT * FROM giftcodes WHERE code = ?").get(code); if (!gift) { await bot.sendMessage(chatId, `⛔ Không tìm thấy giftcode: ${code}`); } else { db.prepare("UPDATE giftcodes SET is_active = 0 WHERE code = ?").run(code); await bot.sendMessage(chatId, `✔️ Đã vô hiệu hóa giftcode *${code}*.`, { parse_mode: "Markdown" }); } resetState(telegramId); return; }
          ...
          case "adm_clearwager_id": {
            ...
            resetState(telegramId);
            await bot.sendMessage(chatId, `✔️ Đã miễn vòng cược cho ${u.first_name ?? u.username ?? "user"} (ID: ${u.id}).\nVòng cược trước đó: *${formatNumber(oldWager)}* → *0*`, { parse_mode: "Markdown" });
            return;
          }
      ...
      switch (state.step) {
        ...
        case "awaiting_bank_owner": {
          ...
          resetState(telegramId);
          await bot.sendMessage(chatId, `✔️ Đã liên kết ngân hàng thành công!\n🏛️ ${state.bankName}\n💳 ${state.bankAccount}\n🎩 ${text.toUpperCase()}`, mainMenuKeyboard()); return;
        }
      }
    } catch (e) { console.error(e); }
  });
  ...
  bot.on("polling_error", async (error: any) => {
    ...
    logger.error({ error: msg }, "Lỗi polling Telegram bot");
    // 409: instance khác đang getUpdates → tạm dừng rồi thử lại
    if (/409|Conflict|terminated by other getUpdates/i.test(msg)) {
      try {
        await bot.stopPolling({ cancel: true } as any);
        await new Promise((r) => setTimeout(r, 5000));
        await bot.deleteWebHook({ drop_pending_updates: true } as any);
        await bot.startPolling({ restart: true });
        logger.info("Đã khôi phục polling sau 409");
      } catch (e: any) {
        logger.error({ err: String(e?.message || e) }, "Khôi phục polling thất bại");
      }
    }
  });
