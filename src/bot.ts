      ...
      for (const adminId of ADMIN_IDS) {
        try {
          await bot.sendMessage(adminId,
            ...
            `🕐 Lúc: ${vnTime()}`,
            { parse_mode: "Markdown", reply_markup: { inline_keyboard: [[{ text: "✅ Duyệt", callback_data: `dep_approve_${depId}` }, { text: "❌ Từ chối", callback_data: `dep_reject_${depId}` }]] } }
          );
        } catch {}
      }
    } catch (e) { console.error(e); }
    ...
    try {
      if (data.startsWith("adm_")) {
        ...
        if (data === "adm_toggle_fakebot") {
          ...
          await bot.answerCallbackQuery(query.id, {
            text: !current ? "✅ Đã bật bot ảo" : "❌ Đã tắt bot ảo",
            show_alert: true,
          });
          return;
        }
